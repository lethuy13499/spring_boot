...
//	List<Question> findAllByOrderByIdDesc();
	@Query(value = "select *from questions q order by q.id desc", nativeQuery = true)
	List<Question> getAllOrderByIdDesc();

	@Query(value = "select *from questions where ( subject_id=:idSubject and id not in (:listSelected))", nativeQuery = true)
	List<Question> getListQuestionOfSubject(@Param("idSubject") int idSubject,
			@Param("listSelected") List<Integer> listSelected);
	...
	List<Question> getListRestQuestion(@Param("idChapter") int idChaper, @Param("idDomain") int idDomain,
			@Param("listSelect") List<Integer> listSelected, @Param("key") String key,
			@Param("idSubject") int subjectId);
	...
	List<Question> getListRestQuestionByKey(@Param("listSelect") List<Integer> listSelected, @Param("key") String key,
			@Param("idSubject") int subjectId);
	...
	@Query(value = "select *from questions where subject_id=:idSubject and ("
			+ " id NOT IN (:listSelect))", nativeQuery = true)
	List<Question> getListRestQuestionByListSelect(@Param("listSelect") List<Integer> listSelected,
			@Param("idSubject") int subjectId);
	...
	List<Object> getListQuestionMinimumChapter(@Param("chapterId") int idChapter,
			@Param("numberOfChaper") int numberOfChapter, @Param("listSelected") List<Integer> listSelected,
			@Param("idSubject") int subjectId);
	...
	List<Object> getListQuestionMinimumDoamin(@Param("domainId") int idDomain,
			@Param("numberOfDomain") int numberOfDomain, @Param("listSelected") List<Integer> listSelected,
			@Param("idSubject") int subjectId);
	...
	List<Object> getListQuestionRandomByChapTerAndDomain(@Param("subjectId") int subjectId,
			@Param("domainName") int domainName, @Param("chapterName") int chapterName,
			@Param("numberQuestion") int numberQuestion);
	...
	@Query(value = " select q.id "
			...
			+ "where q.code like concat('%',:key,'%') " + "or s.name like concat('%',:key,'%') "
			+ "or d.name like concat('%',:key,'%') " + "or c.name like concat('%',:key,'%') "
			+ "or q.time like concat('%',:key,'%') " + "or q.title like concat('%',:key,'%') "
			+ "order by q.id desc", nativeQuery = true)
	List<Integer> search(@Param("key") String key);


	@Query(value = "select a.id from questions as a where (subject_id=:idSubject and a.domain_id =:idDomain and a.chapter_id=:idChapter) Order By RAND() LIMIT :numQuestion", nativeQuery = true)
	List<Integer> getListQuestionRandom(@Param("idSubject") int subjectId, @Param("idDomain") int idDomain,
			@Param("idChapter") int chapterId, @Param("numQuestion") int numQuestion);

	@Query(value = "SELECT count(id) as slcauhoi,domain_id as namedomain  FROM testing_online.questions where subject_id =:subjectId group by domain_id;", nativeQuery = true)
	List<Object> getNumberQuestionOfDomain(@Param("subjectId") int subjectId);

	@Query(value = "SELECT count(id) as slcauhoi,chapter_id as namechapter  FROM testing_online.questions where subject_id =:subjectId group by chapter_id;", nativeQuery = true)
	List<Object> getNumberQuestionOfChapter(@Param("subjectId") int subjectId);


	
	@Query(value = "select a.id from questions as a where (subject_id=:idSubject and a.domain_id REGEXP :idDomain and a.chapter_id REGEXP :idChapter) Order By RAND() LIMIT :numQuestion", nativeQuery = true)
	List<Integer> getListQuestionRandom(@Param("idSubject") int subjectId,@Param("idDomain") String idDomain,@Param("idChapter") String chapterId,
			@Param("numQuestion") int numQuestion
		);
	
	@Query(value = "select count(id) from questions where subject_id =:subjectId and content like %:keyword% or title like %:keyword%", nativeQuery = true)
	int countSearchQuestionBySubject(@Param("subjectId") int subjectId,@Param("keyword") String keyword);
